...
async function askSingleChoiceRpc(
	...
	const otherLabel = getOtherLabel(options);
	const labels = [...options.map((option, index) => `${index + 1}. ${option.label}`), otherLabel];
	...
	if (selected === otherLabel) {
		...
		return trimmed ? { type: "other", label: trimmed, value: trimmed } : null;
	}
...
async function askMultiChoiceRpc(
	...
	const prompt = [
		...
		context ? `\n${context}` : "",
		"",
		"Select one or more answers by typing numbers separated by commas.",
		`Type any custom text to use ${otherLabel}.`,
		"",
		choices,
	].join("\n");
	const response = await ctx.ui.editor(prompt);
	if (response === undefined) return null;

	const answers: AskAnswer[] = [];
	const customParts: string[] = [];
	for (const rawPart of response.split(/[,\n]/)) {
		...
		const option = options[index];
		if (option) {
			answers.push({ type: "option", label: option.label, value: option.value, index: index + 1 });
		}
	}

	const custom = customParts.join(", ").trim();
	if (custom) answers.push({ type: "other", label: custom, value: custom });
